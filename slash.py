...
# yt command
@m.client.tree.command(name="yt",
                       description="Add youtube channel to show new video")
async def yt(interaction: discord.Interaction, channel: str):
  ...
  if (connection.cekusername_yt(username, interaction.guild.id) == False
    ...
    embed = discord.Embed(
      title="Announcement",
      description=
      f"Hello {interaction.user.mention} Username Already exist or Max Limit 🚨 {m.emot[9]}"
    )
    ...
    if check_username == True:
      ...
      if (connection.insert_yt(data) == True):
        embed = discord.Embed(
          title="Announcement",
          description=
          f"Yey {interaction.user.mention} user added successfully 🙌🏼 {m.emot[19]}"
        )
        await interaction.response.send_message(embed=embed)
    else:
      embed = discord.Embed(
        title="Announcement",
        description=
        f"Hello {interaction.user.mention} Username Not Found , Please Add Username Youtube (Without @) 🔴 {m.emot[19]}"
      )
      await interaction.response.send_message(embed=embed)


# show yt command
@m.client.tree.command(
  name="show",
  description="Show All Youtube Channel or Show News About Football")
...
async def show(interaction: discord.Interaction,
               choices: discord.app_commands.Choice[str]):
  ...
  if choices.name == "yt":
    ...
    if len(data) == 0:
      ...
      no = 1
      for i in data:
        if(str(i['server_id']) == str(interaction.guild.id)):
    ...
    if len(data) == 0:
      await interaction.response.send_message(
        content="Hello {0.user.mention} No Data 🚀 {1}".format(
          interaction, m.emot[num - 1]))
    else:
      ...
      if len(check_server) == 0:
        embed = discord.Embed(
          title="Announcement",
          description=
          f"```fix\nyour channel has not turned on the news schedule, please turn it on with the command !jadwal bola {m.emot[num - 1]}```",
          color=0x00ff00)
        await interaction.response.send_message(embed=embed)
      else:
        ...
        embed = discord.Embed(
          title="Announcement",
          description=
          f"Your channel has already turned on the news schedule, please check the channel: {channel.mention}",
          color=0x00ff00)
        await interaction.response.send_message(embed=embed)
  else:
    embed = discord.Embed(title="Announcement",
                          description="```fix\nPlease Enter yt or bola```",
                          color=0x00ff00)
    await interaction.response.send_message(embed=embed)
...
@m.client.tree.command(
  name="delete",
  description=
  "Delete  Youtube Channel or Delete News About Football From This Server")
@discord.app_commands.choices(choices=[
  discord.app_commands.Choice(name="yt", value="yt"),
  discord.app_commands.Choice(name="bola", value="bola")
])
  ...
  if (choices.name == "yt"):
    if (channel == None):
      embed = discord.Embed(
        ...
        description="```diff\n-Please Enter Channel Youtube (Without @)```",
        color=0x00ff00)
      await interaction.response.send_message(embed=embed)
    else:
      username = channel
      if (connection.cekusername_yt(username, interaction.guild.id) == True):
        ...
        embed = discord.Embed(
          title="Delete Youtube Channel",
          description=f"```CSS\nuser successfully deleted {m.emot[num]}```",
          color=0x00ff00)
        await interaction.response.send_message(embed=embed)
  elif (choices.name == "bola"):
    server = interaction.guild.id
    ...
    if len(check_server) == 0:
      embed = discord.Embed(
        title="Delete Soccer Schedule",
        description=
        f"```yaml\nYour channel has not turned on the news schedule, please turn it on with the command !jadwal bola {m.emot[num]}```",
        color=0x00ff00)
      await interaction.response.send_message(embed=embed)
    else:
      ...
      embed = discord.Embed(
        title="Delete Soccer Schedule",
        description=
        f"```CSS\nBall schedule has been successfully deleted {m.emot[num]}```",
        color=0x00ff00)
      await interaction.response.send_message(embed=embed)
...
async def help(interaction: discord.Interaction):
  ...
  embed.add_field(
    name="Show <options>",
    value=
    "```diff\n+Show Youtube Channel or Show News About Football, available options : bola,yt```\nexample : !show yt or !show bola",
    inline=False
  )
...
@discord.app_commands.choices(
  choices=[discord.app_commands.Choice(name="bola", value="bola")])
async def jadwal(interaction: discord.Interaction,
                 choices: discord.app_commands.Choice[str]):
  num = random.randint(0, 25)
  ...
  if (choices.name == "bola"):
    ...
    if len(data) == 0:
      embed = discord.Embed(
        title="Enable Soccer Schedule",
        description=
        f"```CSS\nThere are no matches scheduled for today. {m.emot[num]}```",
        color=0x00ff00)
      await interaction.followup.send(embed=embed)
    else:
      today = a.datetime.datetime.today() + a.datetime.timedelta(days=1)
      ...
      if insData == True:
        ...
        embed = discord.Embed(
          title="Enable Soccer Schedule",
          description=
          f"The ball schedule is already active on the channel {interaction.channel.mention} {m.emot[num]}",
          color=0x00ff00)
        await interaction.followup.send(embed=embed)
